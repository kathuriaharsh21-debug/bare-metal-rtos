...
Source: [`build/example/low_power/example.cpp`](https://github.com/SuperTinyKernel-RTOS/stk/blob/main/build/example/low_power/example.cpp)

Eclipse project: [`build/example/project/eclipse/stm/low_power-stm32f407g-disc1`](c)

> A complete walkthrough of `example.cpp`: how the demo is structured, how the three-tier low-power strategy works, and how to tune or port it to your own project.
...
```
example.cpp        – Main demo: tasks, low-power overrider, entry point
...
By subclassing `IPlatform::IEventOverrider` and registering an instance with `kernel.GetPlatform()->SetEventOverrider(...)`, the application completely replaces the default idle path. Returning `true` signals to the platform driver that the application handled the idle period; returning `false` tells the driver to re-enter its own idle path after the call.

The `LowPowerOverrider` class in `example.cpp` implements a three-tier strategy based on the value of `sleep_ticks`:
...
Setting `STK_TICKLESS_IDLE (1)` in `stk_config.h` enables `KERNEL_TICKLESS` in the kernel mode flags:

```cpp
const uint8_t KernelMode = KERNEL_STATIC | KERNEL_SYNC |
    (STK_TICKLESS_IDLE ? KERNEL_TICKLESS : 0);
```
...
Three `LedTask<ACCESS_PRIVILEGED>` instances share a single `stk::sync::EventFlags` object (`g_TaskFlags`). Each task:

1. Waits for its own flag bit with `EventFlags::Wait(m_my_flag, OPT_WAIT_ANY)`.
2. Atomically switches the active LED inside a `CriticalSection::ScopedLock`.
3. Sleeps for exactly 1 second using `stk::SleepUntil(g_Timeline += GetTicksFromMs(1000))` — drift-free because the deadline is absolute, not relative.
4. Sets the next task's flag bit and loops.
...
**Running → Suspended**
```cpp
kernel->Suspend();          // stops SysTick; returns ticks to next wake-up
g_KernelSuspended = true;
Board::LedSwitchOnExclusive(Led::BLUE);  // BLUE LED = system paused
```

**Suspended → Running**
```cpp
kernel->Resume(0);          // restart scheduling; 0 = "woke from indefinite sleep"
g_KernelSuspended = false;
Board::LedInit();           // all LEDs off; tasks will control them again
```
...
```cpp
// Kernel mode flags
const uint8_t KernelMode = KERNEL_STATIC    // tasks run forever (while(true))
                         | KERNEL_SYNC      // enables EventFlags, Pipe, etc.
                         | KERNEL_TICKLESS; // suppresses SysTick during idle

// 4 user tasks + TimerHost's internal tasks
...
| Step | Call | Reason |
|---|---|---|
| 1 | `Board::*Init()` | Peripherals ready before any task accesses them |
| 2 | `kernel.Initialize()` | Must precede task/timer registration |
| 3 | `timer_host.Initialize(&kernel, ACCESS_PRIVILEGED)` | Must follow `kernel.Initialize()`, precede `kernel.Start()` |
| 4 | `timer_host.Start(adc_timer, period, period)` | Timer registered before scheduler starts |
| 5 | `kernel.GetPlatform()->SetEventOverrider(&lp_overrider)` | Must precede `kernel.Start()` |
| 6 | `kernel.AddTask(&task_*)` | All tasks added before start |
| 7 | `kernel.Start()` | Never returns in `KERNEL_STATIC` mode |
...
A compile-time assert inside `OnSleep` enforces this:
...
```cpp
const uint32_t adc_period = stk::GetTicksFromMs(2000); // currently 2 s
